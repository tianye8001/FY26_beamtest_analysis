...
//Bool_t process_tree_solid_ec(TTree *tree_solid_ec,TTree *tree_solid_ec_ps,double &Eend_ec_sum,double &Eend_ec_ps_sum,double &Eend_ec_Etotal, double &Eend_ec_ps_Etotal,double *Eend_ec,double *Eend_ec_ps)
Bool_t process_tree_solid_ec(TTree *tree_solid_ec,TTree *tree_solid_ec_ps,double &Eend_ec_sum,double &Eend_ec_ps_sum,double *Eend_ec,double *Eend_ec_ps)
{
    for (UInt_t j=0;j<solid_ec_hitn->size();j++) {
       //cout << j<< " solid_ec " << " !!! " << solid_ec_hitn->at(j) << " " << solid_ec_id->at(j) << " " << solid_ec_pid->at(j) << " " << solid_ec_mpid->at(j) << " " << solid_ec_tid->at(j) << " " << solid_ec_mtid->at(j) << "track E= " << solid_ec_trackE->at(j) << " " << solid_ec_totEdep->at(j) << " " << solid_ec_avg_x->at(j) << " " << solid_ec_avg_y->at(j) << " " << solid_ec_avg_z->at(j) << " " << solid_ec_avg_lx->at(j) << " " << solid_ec_avg_ly->at(j) << " " << solid_ec_avg_lz->at(j) << " " << solid_ec_px->at(j) << " " << solid_ec_py->at(j) << " " << solid_ec_pz->at(j) << " " << solid_ec_vx->at(j) << " " << solid_ec_vy->at(j) << " " << solid_ec_vz->at(j) << " " << solid_ec_mvx->at(j) << " " << solid_ec_mvy->at(j) << " " << solid_ec_mvz->at(j) << " " << solid_ec_avg_t->at(j) << endl;    
      ...
      // if(solid_ec_pz->at(j)>0){
       Eend_ec_sum += solid_ec_totEend->at(j);
       //Eend_ec_sum += solid_ec_totEdep->at(j);
       //Eend_ec_Etotal += solid_ec_trackE->at(j);
       Eend_ec[component_ID] +=solid_ec_totEend->at(j);
       //Eend_ec[component_ID] +=solid_ec_totEdep->at(j);
       //cout<<"component_ID="<<component_ID<<"  x="<<solid_ec_avg_lx->at(j)<<"  y="<<solid_ec_avg_ly->at(j)<<endl;   
//       cout<<"j="<<j<<"solid_ec_totEend="<<solid_ec_totEend->at(j)<<"component_ID="<<component_ID<<"Eend_ec_sum="<<Eend_ec_sum<<endl; 
    // }
    }
    
    for (UInt_t j=0;j<solid_ec_ps_hitn->size();j++) {
       int detector_ID_ps=solid_ec_ps_id->at(j)/1000000;
       int subdetector_ID_ps=(solid_ec_ps_id->at(j)%1000000)/100000;
       int subsubdetector_ID_ps=((solid_ec_ps_id->at(j)%1000000)%100000)/10000;
       int component_ID_ps=solid_ec_ps_id->at(j)%10000; 
     // if(solid_ec_ps_pz->at(j)>0){ 
      Eend_ec_ps_sum += solid_ec_ps_totEend->at(j);
      //Eend_ec_ps_sum += solid_ec_ps_totEdep->at(j);
     // Eend_ec_ps_Etotal += solid_ec_ps_trackE->at(j);
      Eend_ec_ps[component_ID_ps] += solid_ec_ps_totEend->at(j);
      //Eend_ec_ps[component_ID_ps] += solid_ec_ps_totEdep->at(j);
    // }
    }
